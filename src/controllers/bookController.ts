...
export class BookController {
  ...
  getAll = async (
    req: Request<{}, unknown, {}, { genre?: string }>,
    res: Response
  ): Promise<void> => {
  ...
  create = async (
    req: Request<{}, unknown, CreateBookRequest>,
    res: Response
  ): Promise<void> => {
  ...
  update = async (
    req: Request<{ id: string }, unknown , UpdateBookRequest>,
    res: Response
  ): Promise<void> => {
  ...
  getDiscountedPrice = async (
    req: Request<{}, unknown, unknown, { genre?: string; discount?: string }>,
    res: Response
  ): Promise<void> => {
    const genre = req.query.genre;
    if (!genre) {
      throw new ValidationError("Query parameter 'genre' is required");
    }

    const discount = this.parseDiscount(req.query.discount);
    const result = await this.service.calculateDiscountedPrice(genre, discount);
  ...
  private parseId(rawString: string): number {
    ...
    if (!Number.isInteger(id) || id < 1) { // <1 could be moved into service level
      throw new ValidationError(`Invalid id: ${rawString}`);
    }
    return id;
  }

  // throws if raw is missing or not a valid number
  private parseDiscount(raw: string | undefined): number {
    if (raw === undefined) {
      throw new ValidationError("Query parameter 'discount' is required");
    }
    const discount = Number(raw);
    if (Number.isNaN(discount)) {
      throw new ValidationError(`Invalid discount: "${raw}"`);
    }
    return discount;
}
}